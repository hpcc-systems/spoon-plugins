        ...
        try{
            ...
            for(int i = 0; i < rec.getRecords().size(); i++){
                ...
                try{
                       ...
                       if(ob.getColumnType().startsWith("String")){
            ...
            bucket.setColumnName("Bucket");
            bucket.setColumnType("REAL"); 
            bucket.setDefaultValue("-1");
            recordList.addRecordBO(bucket);
            
            bucket = new RecordBO();
            bucket.setColumnName("Minval");
            bucket.setColumnType("REAL");
            bucket.setDefaultValue("0");
            recordList.addRecordBO(bucket);
            
            bucket = new RecordBO();
            bucket.setColumnName("Maxval");
            bucket.setColumnType("REAL");
            bucket.setDefaultValue("0");
            recordList.addRecordBO(bucket);
	...
	private boolean validate(){
    	...
    	if(this.jobEntryName.getText().equals("")){
   		...
   		if(this.OutName.equals("")){
   			isValid = false;
   			errors += "You need to the output dataset name!\r\n";
   		}
